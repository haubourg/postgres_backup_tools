...
{
	SUFFIX=$1
	FINAL_BACKUP_DIR=$BACKUP_DIR"`date +\%Y-\%m-\%dT\%H:\%M-$SUFFIX/`"
	...
	then
		    ...
		    set -o pipefail
		    if ! pg_dumpall -g -h "$HOSTNAME" -U "$USERNAME" -p "$PORTNUMBER" | gzip > $FINAL_BACKUP_DIR"globals".sql.gz.in_progress; then
	...
	do
		...
		then
			...
			set -o pipefail
			if ! pg_dump -Fp -h "$HOSTNAME" -U "$USERNAME" "$DATABASE" -p "$PORTNUMBER" | gzip > $FINAL_BACKUP_DIR"$DATABASE".sql.gz.in_progress; then
		...
		then
			...
			if ! pg_dump -Fc -h "$HOSTNAME" -U "$USERNAME" "$DATABASE" -p "$PORTNUMBER" -f $FINAL_BACKUP_DIR"$DATABASE".custom.in_progress; then
